...
<label class="relative inline-flex items-center self-start mt-[0.2em] me-16">
  <input
    ...
    name="{{ $name }}"
    id="{{ $id }}"
    {{ $attributes->merge(['class' => 'appearance-none rounded-full size-24 lg:size-32 ring-0 focus:ring-0 !ring-offset-0 !outline-none !border-fern border-2 lg:border-3 text-white bg-white accent-none']) }}>
